...
@router.get("/staging/report/download", response_class=PlainTextResponse)
def download_import_report(db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    """
    Generate and download the Import Report summarizing all anomalies and actions taken.
    """
    records = db.query(models.StagingExpense).all()
    ...
    for r in records:
        ...
        flags_desc = []
        if r.anomaly_flags:
            errors = r.anomaly_flags.get("errors", [])
            warnings = r.anomaly_flags.get("warnings", [])
            if errors or warnings:
                has_flags = True
                for err in errors:
                    flags_desc.append(f"Error: {err.get('message', 'Unknown Error')}")
                for wrn in warnings:
                    flags_desc.append(f"Warning: {wrn.get('message', 'Unknown Warning')}")
        
        if has_flags or r.status == "flagged":
            flagged_records.append((r, flags_desc))

    report_lines = [
        "==================================================",
        "            SHARED EXPENSES IMPORT REPORT         ",
        "==================================================",
        f"Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        f"Generated by: {current_user.email}",
        f"Total Rows Processed: {total_processed}",
        f"Total Anomalies Flagged: {len(flagged_records)}",
        "",
        "--------------------------------------------------",
        "DETAILED ANOMALY LOG & ACTIONS TAKEN",
        "--------------------------------------------------",
    ]
    
    if not flagged_records:
        report_lines.append("No anomalies detected in this ingestion batch.")
    else:
        for idx, (r, flags) in enumerate(flagged_records, 1):
            report_lines.append(f"{idx}. Row ID: {r.id}")
            report_lines.append(f"   Description: {r.raw_description or 'N/A'}")
            report_lines.append(f"   Date: {r.raw_date or 'N/A'}")
            report_lines.append(f"   Amount: {r.raw_amount or '0.00'} {r.raw_currency or 'INR'}")
            report_lines.append(f"   Payer: {r.raw_paid_by or 'N/A'}")
            report_lines.append(f"   Detected Anomalies:")
            if flags:
                for f_desc in flags:
                    report_lines.append(f"     - {f_desc}")
            else:
                report_lines.append(f"     - Flagged for review (missing details or formatting issue)")
            
            action_taken = "Unknown"
            if r.status == "pending":
                action_taken = "Pending Review (Re-validation requested)"
            elif r.status == "flagged":
                action_taken = "Pending Action (Awaiting user review)"
            elif r.status == "approved":
                action_taken = "Approved (Force-approved or corrected & approved, promoted to production)"
            elif r.status == "discarded":
                action_taken = "Discarded (Rejected & deleted from active queue)"
            elif r.status == "valid":
                action_taken = "Corrected & Validated (Ready for approval)"
                
            report_lines.append(f"   Action Taken: {action_taken}")
            report_lines.append("")
            
    report_lines.append("==================================================")
    report_lines.append("End of Import Report.")
    ...
    return PlainTextResponse(
        content=report_text,
        headers={
            "Content-Disposition": "attachment; filename=import_report.txt"
        }
    )
...
def discard_staging_expense(row_id: int, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    """
    Discard (mark as discarded) the staging expense with the given ID.
    """
    ...
    if staging.status in ("approved", "discarded"):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Staging expense ID {row_id} has already been processed (status: {staging.status})."
        )
    
    staging.status = "discarded"
    db.commit()
...
def approve_staging_expense(row_id: int, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    ...
    # 11. Clean up StagingExpense record (mark as approved)
